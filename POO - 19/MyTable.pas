...
uses System.Variants, Winapi.Windows, System.SysUtils, System.Classes, Data.DB,
  Data.DBCommon, Data.DBCommonTypes, System.Generics.Collections,
  System.AnsiStrings;
...
const
  ...
  BlobTypeMap: array of TFieldType = (
...
{ BDE error information type }
  ...
  public
    constructor Create(Owner: EDBEngineError; ErrorCode: Integer;
...
{ TBDECallback }
  ...
  public
    constructor Create(AOwner: TObject; Handle: THandle; CBType: CBType;
...
{ TSession }
  ...
  private
    ...
    function FindDatabaseHandle(const DatabaseName: string): Variant;
...
{$HPPEMIT '#endif'}
    ...
    procedure SetHandle(Value: Variant);
  ...
  public
    ...
    property Handle: Variant read FHandle write SetHandle;
...
{ TTable }
  ...
  private
    FOwner: TMyTable;
  public
    constructor Create(AOwner: TMyTable);
  ...
  public
    ...
    procedure GotoCurrent(Table: TMyTable);
...
{ TBatchMove }
  ...
  published
    ...
    property Destination: TMyTable read FDestination write FDestination;
...
begin
  ...
  if System.AnsiStrings.StrLen(DrvName) > 0 then OsLdLoadBySymbName(DrvName, Result);
end;
...
begin
  ...
  begin
    Len := MaxLen;
    if SysLocale.FarEast and (System.AnsiStrings.ByteType(AnsiStr, Len) = mbLeadByte) then
      Dec(Len);
  end;
  NativeStr[Len] := #0;
...
var
  Len: Integer;
begin
  Len := System.AnsiStrings.StrLen(NativeStr);
...
begin
  Result := (System.AnsiStrings.StrIComp(AType, szPARADOX) = 0) or
    (System.AnsiStrings.StrIComp(AType, szDBASE) = 0) or
    (System.AnsiStrings.StrIComp(AType, szFOXPRO) = 0);
...
function TSession.FindDatabaseHandle(const DatabaseName: string): Variant;
var
  I: Integer;
  DB: TDatabase;
begin
...
begin
  ...
  try
    List.Clear;
    System.AnsiStrings.StrPLCopy(SAlias, AnsiString(AliasName), SizeOf(SAlias) - 1);
    ...
    if System.AnsiStrings.StrIComp(Desc.szDbType, szCFGDBSTANDARD) = 0 then
    begin
...
function TSession.GetHandle: THandle;
begin
  if FHandle <> nil then
      ...
      try
        ...
        begin
          if Extensions and (Desc.szExt[0] <> #0) then
            System.AnsiStrings.StrCat(System.AnsiStrings.StrCat(Desc.szName, '.'), Desc.szExt);
...
begin
  ...
  try
    ...
    System.AnsiStrings.StrPLCopy(Env.szLang, AnsiString(SIDAPILangID), SizeOf(Env.szLang) - 1);
    Status := DbiInit(@Env);
  ...
  try
    ...
    begin
      if not Assigned(DefaultSession.FSMClient) then
        CoCreateInstance(Class_SMClient, nil, CLSCTX_INPROC_SERVER, Variant,
  ...
  begin
    ...
    if FSQLHourGlass then
      FCallbacks.Add(TBDECallback.Create(Self, nil, cbSERVERCALL,
        @FVariant, SizeOf(Variant), ServerCallBack, False));

    FCallbacks.Add(TBDECallback.Create(Self, nil, cbDBLOGIN,
      @FCBDBLogin, SizeOf(Variant), DBLoginCallBack, False));
  end else
  begin
...
begin
  try
    Data := @Variant(CBInfo).pszTrace;
    FSMClient.AddStatement(Data, System.AnsiStrings.StrLen(PAnsiChar(Data)));
  except
    SetTraceFlags([]);
  end;
  Result := cbrUSEDEF;
    ...
    begin
      ...
      if P <> 0 then
        ...
        begin
          Inc(FieldNo);
          iFldNum := FieldNo;
          System.AnsiStrings.StrPLCopy(szName, Copy(S, 1, P - 1), SizeOf(szName) - 1);
          iFldType := fldZSTRING;
          iOffset := BufPtr - FBuffer;
          iLen := Length(S) - P + 1;
          System.AnsiStrings.StrCopy(BufPtr, PAnsiChar(Copy(S, P + 1, 255)));
...
begin
  ...
  FSQLBased := System.AnsiStrings.StrIComp(Buffer, szCFGDBSTANDARD) <> 0;
        ...
        try
          ...
          try
            ...
            begin
              ...
              System.AnsiStrings.StrCopy(ObjectContextDesc.szName, szMTXCONTEXTOBJ);
...
procedure TDatabase.SetHandle(Value: Variant);
var
  DBSession: THandle;
begin
...
begin
  ...
  case LType of
    ...
    ftArray:
      begin
        I := FieldID;
        System.AnsiStrings.StrCat(System.AnsiStrings.StrCopy(FieldDescs[DescNo].szName, FieldDesc.szName),'[0]');
...
begin
  Result := System.AnsiStrings.StrLen(PAnsiChar(Src));
  if ToOem then
...
begin
  ...
  begin
    ...
    try
      CalculateFields(NativeInt(TempBuffer));
...
function TBDEDataSet.ConstraintCallBack(Req: DsInfoReq;
  ...
  begin
    ...
    begin
      System.AnsiStrings.StrCopy(ADataSources.szSQLExprImport, PAnsiChar(AnsiString(Field.ImportedConstraint)));
      System.AnsiStrings.StrCopy(ADataSources.szSQLExprCustom, PAnsiChar(AnsiString(Field.CustomConstraint)));
      System.AnsiStrings.StrCopy(ADataSources.szErrStrCustom, PAnsiChar(AnsiString(Field.ConstraintErrorMessage)));
      System.AnsiStrings.StrCopy(ADataSources.szErrStrImport, PAnsiChar(AnsiString(Field.ConstraintErrorMessage)));
  ...
  begin
    System.AnsiStrings.StrCopy(ADataSources.szSQLExprImport, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].ImportedConstraint)));
    System.AnsiStrings.StrCopy(ADataSources.szSQLExprCustom, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].CustomConstraint)));
    System.AnsiStrings.StrCopy(ADataSources.szErrStrCustom, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].ErrorMessage)));
    System.AnsiStrings.StrCopy(ADataSources.szErrStrImport, PAnsiChar(AnsiString(Constraints[ADataSources.iNumElem - 1].ErrorMessage)));
  end;
  ...
  begin
    ...
    begin
      System.AnsiStrings.StrCopy(ADataSources.szSQLExprImport, PAnsiChar(AnsiString(Field.DefaultExpression)));
      Result := True;
    end;
  end;
...
function TDBDataSet.GetDBHandle: Variant;
begin
...
constructor TIndexFiles.Create(AOwner: TMyTable);
begin
  inherited Create;
  FOwner := AOwner;
end;
...
function TMyTable.GetHandle(const IndexName, IndexTag: string): HDBICur;
const
  OpenModes: array[Boolean] of DbiOpenMode = (dbiReadWrite, dbiReadOnly);
  ShareModes: array[Boolean] of DbiShareMode = (dbiOpenShared, dbiOpenExcl);
var
...
function TMyTable.CreateHandle: HDBICur;
var
  CursorLocale: TLocale;
...
function TMyTable.GetLanguageDriverName: string;
var
  TblName: DBITBLNAME;
  LdName: DBINAME;
...
function TMyTable.SetTempLocale(ActiveCheck: Boolean): TLocale;
var
  LName: string;
  TempLocale: TLocale;
begin
...
procedure TMyTable.RestoreLocale(LocaleSave: TLocale);
begin
...
procedure TMyTable.PrepareCursor;
var
  IndexName, IndexTag: string;
begin
  if IsXBaseTable then
...
procedure TMyTable.DefChanged(Sender: TObject);
begin
  StoreDefs := True;
end;

procedure TMyTable.InitFieldDefs;
var
...
procedure TMyTable.DestroyHandle;
begin
  DestroyLookupCursor;
...
procedure TMyTable.DecodeIndexDesc(const IndexDesc: IDXDesc;
  var Source, Name, FieldExpression, DescFields: string;
  var Options: TIndexOptions);
...
procedure TMyTable.EncodeIndexDesc(var IndexDesc: IDXDesc;
  const Name, FieldExpression: string; Options: TIndexOptions;
  const DescFields: string);
...
procedure TMyTable.AddIndex(const Name, Fields: string; Options: TIndexOptions;
  const DescFields: string);
var
  IndexDesc: IDXDesc;
  LocaleSave: TLocale;
...
procedure TMyTable.DeleteIndex(const Name: string);
var
  IndexName, IndexTag: string;
begin
  if Active then
...
function TMyTable.GetIndexFieldNames: string;
begin
  if FFieldsIndex then Result := FIndexName else Result := '';
end;

function TMyTable.GetIndexName: string;
begin
  if FFieldsIndex then Result := '' else Result := FIndexName;
end;

procedure TMyTable.GetIndexNames(List: TStrings);
begin
  IndexDefs.Update;
  IndexDefs.GetItemNames(List);
end;

procedure TMyTable.GetIndexParams(const IndexName: string;
...
procedure TMyTable.SetIndexDefs(Value: TIndexDefs);
begin
  IndexDefs.Assign(Value);
end;

procedure TMyTable.SetIndex(const Value: string; FieldsIndex: Boolean);
var
  IndexName, IndexTag: string;
begin
...
procedure TMyTable.SetIndexFieldNames(const Value: string);
begin
  SetIndex(Value, Value <> '');
end;

procedure TMyTable.SetIndexName(const Value: string);
begin
  SetIndex(Value, False);
end;

procedure TMyTable.SetIndexFiles(Value: TStrings);
begin
  FIndexFiles.Assign(Value);
end;

procedure TMyTable.OpenIndexFile(const IndexName: string);
var
  Buffer: DBINAME;
begin
...
procedure TMyTable.CloseIndexFile(const IndexFileName: string);
var
  IndexName, IndexTag: string;
  Buffer: DBINAME;
begin
...
procedure TMyTable.UpdateIndexDefs;
var
  Opts: TIndexOptions;
...
function TMyTable.IsProductionIndex(const IndexName: string): Boolean;
begin
  Result := True;
  if IsXBaseTable and (IndexName <> '') then
    if AnsiUpperCase(ExtractFileExt(IndexName)) = '.NDX' then
      Result := False
    else Result := AnsiUpperCase(ChangeFileExt(string(NativeTableName), '')) =
      AnsiUpperCase(ChangeFileExt(IndexName, ''));
end;

function TMyTable.GetExists: Boolean;
var
  H: HDBICur;
  E: DBIResult;
begin
...
function TMyTable.FindKey(const KeyValues: array of const): Boolean;
begin
  CheckBrowseMode;
...
procedure TMyTable.FindNearest(const KeyValues: array of const);
begin
  CheckBrowseMode;
  SetKeyFields(kiLookup, KeyValues);
  GotoNearest;
...
function TMyTable.GotoKey: Boolean;
var
...
procedure TMyTable.GotoNearest;
var
...
procedure TMyTable.SetKey;
begin
  SetKeyBuffer(kiLookup, True);
end;

procedure TMyTable.EditKey;
begin
  SetKeyBuffer(kiLookup, False);
end;

procedure TMyTable.ApplyRange;
begin
  CheckBrowseMode;
...
procedure TMyTable.CancelRange;
begin
  CheckBrowseMode;
  UpdateCursorPos;
...
procedure TMyTable.SetRange(const StartValues, EndValues: array of const);
begin
  CheckBrowseMode;
...
procedure TMyTable.SetRangeEnd;
begin
  SetKeyBuffer(kiRangeEnd, True);
end;

procedure TMyTable.SetRangeStart;
begin
  SetKeyBuffer(kiRangeStart, True);
end;

procedure TMyTable.EditRangeEnd;
begin
  SetKeyBuffer(kiRangeEnd, False);
end;

procedure TMyTable.EditRangeStart;
begin
  SetKeyBuffer(kiRangeStart, False);
end;

procedure TMyTable.UpdateRange;
begin
  SetLinkRanges(FMasterLink.Fields);
end;

function TMyTable.GetLookupCursor(const KeyFields: string;
  CaseInsensitive: Boolean): HDBICur;
var
...
procedure TMyTable.DestroyLookupCursor;
begin
...
procedure TMyTable.GotoCurrent(Table: TMyTable);
begin
  CheckBrowseMode;
...
procedure TMyTable.GetDetailLinkFields(MasterFields, DetailFields: TList<TField>);
var
  i: Integer;
  Idx: TIndexDef;
begin
...
procedure TMyTable.GetDetailLinkFields(MasterFields, DetailFields: TList);
var
  i: Integer;
  Idx: TIndexDef;
begin
...
procedure TMyTable.CheckMasterRange;
begin
...
procedure TMyTable.MasterChanged(Sender: TObject);
begin
  CheckBrowseMode;
  UpdateRange;
  ApplyRange;
...
procedure TMyTable.MasterDisabled(Sender: TObject);
begin
  CancelRange;
end;

function TMyTable.GetDataSource: TDataSource;
begin
  Result := FMasterLink.DataSource;
end;

procedure TMyTable.SetDataSource(Value: TDataSource);
begin
  if IsLinkedTo(Value) then DatabaseError(SCircularDataLink, Self);
  FMasterLink.DataSource := Value;
end;

function TMyTable.GetMasterFields: string;
begin
  Result := FMasterLink.FieldNames;
end;

procedure TMyTable.SetMasterFields(const Value: string);
begin
  FMasterLink.FieldNames := Value;
end;

procedure TMyTable.DoOnNewRecord;
var
  I: Integer;
begin
  if FMasterLink.Active and (FMasterLink.Fields.Count > 0) then
...
function TMyTable.BatchMove(ASource: TBDEDataSet; AMode: TBatchMode): Longint;
var
...
procedure TMyTable.CreateTable;
var
  LocaleSave: TLocale;
  IndexDescs: TIndexDescList;
  TableDesc: CRTblDesc;
  ...
  begin
    ...
    if Assigned(PTblType) then System.AnsiStrings.StrCopy(TableDesc.szTblType, PTblType);
    if FTableLevel > 0 then
    begin
      TableDesc.iOptParams := 1;
      System.AnsiStrings.StrCopy(@Level, PAnsiChar(AnsiString(IntToStr(FTableLevel))));
      TableDesc.pOptData := @Level;
      System.AnsiStrings.StrCopy(LvlFldDesc.szName, szCFGDRVLEVEL);
      LvlFldDesc.iLen := System.AnsiStrings.StrLen(Level) + 1;
...
procedure TMyTable.DeleteTable;
begin
  CheckInactive;
  SetDBFlag(dbfTable, True);
  try
...
procedure TMyTable.LockTable(LockType: TLockType);
begin
  SetTableLock(LockType, True);
end;

procedure TMyTable.SetTableLock(LockType: TLockType; Lock: Boolean);
var
  L: DBILockType;
begin
  CheckActive;
...
procedure TMyTable.UnlockTable(LockType: TLockType);
begin
  SetTableLock(LockType, False);
end;

procedure TMyTable.RenameTable(const NewTableName: string);
var
...
procedure TMyTable.EncodeFieldDesc(var FieldDesc: BDEFLDDesc;
  const Name: string; DataType: TFieldType; Size, Precision: Integer);
begin
...
procedure TMyTable.DataEvent(Event: TDataEvent; Info: NativeInt);
begin
...
function TMyTable.GetDriverTypeName(Buffer: PAnsiChar): PAnsiChar;
var
  Length: Word;
begin
  Result := Buffer;
  ...
  if System.AnsiStrings.StrIComp(Buffer, szCFGDBSTANDARD) = 0 then
  begin
    Result := GetTableTypeName;
    if Result <> nil then Result := System.AnsiStrings.StrCopy(Buffer, Result);
  end;
end;

function TMyTable.GetTableTypeName: PAnsiChar;
begin
  if Database.IsSQLBased then Result := nil
  else Result := TableTypeDriverNames[GetTableType];
end;

function TMyTable.GetTableLevel: Integer;
begin
  if Handle <> nil then
...
function TMyTable.GetTableType: TTableType;
var
...
function TMyTable.NativeTableName: PAnsiChar;
begin
...
procedure TMyTable.SetExclusive(Value: Boolean);
begin
  CheckInactive;
  FExclusive := Value;
end;

procedure TMyTable.SetReadOnly(Value: Boolean);
begin
  CheckInactive;
  FReadOnly := Value;
end;

procedure TMyTable.SetTableName(const Value: TFileName);
begin
  if csReading in ComponentState then
    FTableName := Value
...
procedure TMyTable.SetTableType(Value: TTableType);
begin
  CheckInactive;
  FTableType := Value;
end;
...
function TMyTable.PSGetCommandText: string;
begin
  Result := TableName;
end;

function TMyTable.PSGetCommandType: TPSCommandType;
begin
  Result := ctTable;
end;

function TMyTable.PSGetDefaultOrder: TIndexDef;
...
function TMyTable.PSGetIndexDefs(IndexTypes: TIndexOptions): TIndexDefs;
begin
  Result := GetIndexDefs(IndexDefs, IndexTypes);
end;

function TMyTable.PSGetTableName: string;
begin
  Result := TableName;
end;

procedure TMyTable.PSSetParams(AParams: TParams);
...
procedure TMyTable.PSSetCommandText(const CommandText: string);
begin
  if CommandText <> '' then
    TableName := CommandText;
end;

function TMyTable.PSGetKeyFields: string;
var
  i, Pos: Integer;
  IndexFound: Boolean;
begin
      ...
      begin
        ...
        begin
          if DataType in [ftString, ftFixedChar] then
            NativeToAnsiBuf(Locale, CurPtr, CurPtr, System.AnsiStrings.StrLen(CurPtr));
...
begin
  ...
  FParams.Free;
  FDataLink.Free;
  System.AnsiStrings.StrDispose(SQLBinary);
  inherited Destroy;
end;
...
begin
  Stream.WriteBuffer(SQLBinary^, System.AnsiStrings.StrBufSize(SQLBinary));
end;
